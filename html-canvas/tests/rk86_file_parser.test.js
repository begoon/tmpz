...
test("extract name=name after signature", extract_metadata, "#!rk86\n!name=name\n", { name: "name" });

test("extract name=name after signature on the same line", extract_metadata, "#!rk86 !name=name\n", { name: "name" });

test("extract tags from multiple lines", extract_metadata, "#!name=name\n!start=100 !end=200 !entry=300", {
    name: "name",
    start: "100",
    end: "200",
    entry: "300",
...
test("name only", parse_rk86_binary, ["name.rk", [0xe6, 0x11, 0x22, 0x33, 0x44, 0xaa]], {
    name: "name.rk",
    start: 0x1122,
    end: 0x3344,
    entry: 0x1122,
...
test("name with folder", parse_rk86_binary, ["folder/name.rk", [0xe6, 0x22, 0x11, 0x44, 0x33, 0x66]], {
    name: "name.rk",
    start: 0x2211,
    end: 0x4433,
    entry: 0x2211,
...
test("name with URL", parse_rk86_binary, ["https://domain.com/path/name.rk", [0xe6, 0x11, 0x22, 0x33, 0x44, 0xaa]], {
    name: "name.rk",
    start: 0x1122,
    end: 0x3344,
    entry: 0x1122,
...
test("name PVO.GAM with hardcoded entry", parse_rk86_binary, ["PVO.GAM", [0xe6, 0x11, 0x22, 0x33, 0x44, 0xaa]], {
    name: "PVO.GAM",
    start: 0x1122,
    end: 0x3344,
    entry: 0x3400,
...
test("monitor binary with extension .bin", parse_rk86_binary, ["mon32.bin", [0x11, 0x22]], {
    name: "mon32.bin",
    start: 0xfffe,
    end: 0xffff,
    entry: 0xfffe,
...
test("monitor binary without extension", parse_rk86_binary, ["mon32", [0x11, 0x22]], {
    name: "mon32",
    start: 0xfffe,
    end: 0xffff,
    entry: 0xfffe,
...
test("binary with extension .bin", parse_rk86_binary, ["binary.bin", [0x11, 0x22]], {
    name: "binary.bin",
    start: 0,
    end: 1,
    entry: 0,
...
test("regular binary with empty extension", parse_rk86_binary, ["binary", [0x11, 0x22, 0x33]], {
    name: "binary",
    start: 0,
    end: 2,
    entry: 0,
...
test(
    "entry should be taken from tags in RK file",
    parse_rk86_hex,
    ["random", "#!rk86 !name=image.rk !start=0100 !entry=0200\n" + "0000 E6 11 22 11 23 AA BB\n"],
    {
        name: "image.rk",
        start: 0x1122,
        end: 0x1123,
